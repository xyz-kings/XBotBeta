  ...
  async execute(bot, m, args) {
    try {
      ...
      const res = await axios.get(
        `${config.Funv2BaseURL}/xyz/ramalan`,
      ...
      if (!res.data || !res.data.status) {
        return bot.sendMessage(
          m.key.remoteJid,
          { text: "❌ Gagal mengambil data ramalan. Coba lagi nanti." },
          { quoted: m }
        );
      }

      const result = res.data.result;
      let caption = `*🔮 Ramalan Hari Ini*\n\n📛 *Nama:* @${targetJid ? targetJid.split('@')[0] : targetName}\n\n📖 *Ramalan:* ${result.hari_ini}\n\n📝 *Catatan:* ${result.catatan}\n\n${res.data.note ? res.data.note + "\n\n" : ""}_${config.copyright || ""}_`;
      ...
      console.error("[RAMALAN ERROR]", err);
      await bot.sendMessage(
        m.key.remoteJid,
        { text: "❌ Terjadi kesalahan saat memproses ramalan." },
        { quoted: m }
      );
    }
  },
