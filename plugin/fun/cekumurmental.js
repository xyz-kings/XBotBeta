...
module.exports = {
  command: ["cekumurmental"],
  category: "fun",
  description: "Cek umur mental seseorang",
  ...
  async execute(bot, m, args) {
    try {
      ...
      const res = await axios.get(
        `${config.FunBaseURL}/xyz/cekumurmental/user`,
      ...
      let caption = `*🧠 Cek Umur Mental*\n\n📛 *Nama:* @${targetJid ? targetJid.split('@')[0] : targetName}\n🎂 *Umur Mental:* ${res.data.result.umur} tahun\n🧩 *Tingkat Kematangan:* ${res.data.result.kematangan}\n📝 *Deskripsi:* ${res.data.result.deskripsi}\n💭 *Karakteristik:* ${res.data.result.karakteristik}\n\n${res.data.note ? res.data.note + "\n\n" : ""}_${config.copyright || ""}_`;
      ...
      console.error("[CEKUMURMENTAL ERROR]", err);
      await bot.sendMessage(
        m.key.remoteJid,
