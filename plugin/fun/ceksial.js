...
module.exports = {
  command: ["ceksial"],
  category: "fun",
  description: "Cek kesialan seseorang",
  ...
  async execute(bot, m, args) {
    try {
      ...
      const res = await axios.get(
        `${config.FunBaseURL}/xyz/ceksial/user`,
      ...
      let caption = `*☠️ Cek Kesialan*\n\n📛 *Nama:* @${targetJid ? targetJid.split('@')[0] : targetName}\n⭐ *Tingkat Kesialan:* ${res.data.result.score}/100\n⚠️ *Penyebab:* ${res.data.result.penyebab}\n📝 *Deskripsi:* ${res.data.result.deskripsi}\n🙏 *Penangkal:* ${res.data.result.penangkal}\n\n${res.data.note ? res.data.note + "\n\n" : ""}_${config.copyright || ""}_`;
      ...
      console.error("[CEKSIAL ERROR]", err);
      await bot.sendMessage(
        m.key.remoteJid,
